...
# Migrations of the hand-written opencode.json — managed here so the file
    # ends up in the Nix store (which lets services.t3code.opencodeConfigPath
    # be a pure-eval reference, no --impure flag needed).
    # settings = {
    ...
    #       models = {
    #         "gemma4:26b" = { _launch = true; name = "gemma4:26b"; };
    #         "glm-4.7-flash" = { _launch = true; name = "glm-4.7-flash"; };
    #         "qwen3.5" = { _launch = true; name = "qwen3.5"; };
    #       };
    #     };
    #   };
    # };
  ...
  services.t3code = {
    ...
    # Pass through the real opencode.json so the t3code UI shows our
    # custom providers/models (ollama, etc.) instead of an empty default.
    # xdg.configFile."opencode/opencode.json" is generated by programs.opencode
    # (see above), so this is a pure-eval Nix store path.
    # See home-manager/modules/t3code.nix for the upstream bug workaround.
    opencodeConfigPath = config.xdg.configFile."opencode/opencode.json".source;
  };
