...
let
  cfgFileSystems = config.zfs-root.fileSystems;
...
in {
  ...
  }) cfgFileSystems.datasets ++ mapAttrsToList (bindsrc: mountpoint: {
    "${mountpoint}" = {
      device = "${bindsrc}";
      fsType = "none";
      options = [ "bind" "X-mount.mkdir" "noatime" ];
    };
  }) cfgFileSystems.bindmounts ++ map (esp: {
    "/boot/efis/${esp}" = {
  ...
  }) cfgFileSystems.efiSystemPartitions);
