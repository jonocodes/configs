# Configuration for hosts on the home LAN (192.168.200.0/24) behind plex router
#
# Configures split-horizon DNS approach:
# - Prepends local router DNS (192.168.200.1) for internal domain resolution
# - DNS order: 192.168.200.1 (split-horizon) → 100.100.100.100 (Tailscale) → ISP (fallback)
...
{ ... }:
{
  networking.networkmanager.insertNameservers = [ "192.168.200.1" ];
