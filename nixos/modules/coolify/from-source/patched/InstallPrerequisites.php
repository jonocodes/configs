...
class InstallPrerequisites
{
    ...
    public function handle(Server $server)
    {
        ...
        if ($supported_os_type->contains('debian')) {
            ...
            // Just verify the essential tools exist rather than trying to install imperatively.
            $command = $command->merge([
                "echo 'NixOS detected. Prerequisites are managed via configuration.nix.'",
                "command -v curl >/dev/null && echo '  curl: found' || echo '  WARNING: curl not found'",
                "command -v wget >/dev/null && echo '  wget: found' || echo '  WARNING: wget not found'",
                "command -v git >/dev/null && echo '  git: found' || echo '  WARNING: git not found'",
                "command -v jq >/dev/null && echo '  jq: found' || echo '  WARNING: jq not found'",
            ]);
        } else {
