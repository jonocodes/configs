    ...
    public function handle(Server $server)
    {
        ...
        if (isDev() && $server->id === 0) {
            ...
            if ($supported_os_type->contains('debian')) {
                ...
                // NixOS: Docker should be enabled declaratively via configuration.nix.
                // We just verify it's available rather than trying to install imperatively.
                $command = $command->merge([
                    "echo 'NixOS detected. Docker should be enabled in your NixOS configuration:'",
                    "echo '  virtualisation.docker.enable = true;'",
                    "echo 'Checking if Docker is already available...'",
                    'command -v docker >/dev/null && echo "Docker is available." || (echo "ERROR: Docker not found. Add virtualisation.docker.enable = true to your configuration.nix" && exit 1)',
                ]);
            } else {
