...
```nix
{
  inputs = {
    ...
    # Import builder
    lib.url = "git+file:..?dir=lib";
    lib.inputs.nixpkgs.follows = "nixpkgs";

    # Import host flake
    plex.url = "git+file:..?dir=hosts/plex";
...
**Deploy:** `sudo nixos-rebuild switch --flake ./new#plex --impure`

**Note:** `new/flake.nix` uses absolute paths because it's in a subdirectory (Nix limitation). The per-host flakes use relative paths and are portable. To make everything portable, move `new/flake.nix` to the repo root after migration is complete.
...
3. Add to `new/flake.nix`: `newhost.url = "git+file:..?dir=hosts/newhost";` with `.follows`
